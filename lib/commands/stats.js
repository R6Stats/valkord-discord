...
p.invoke = function(msg, args) {
	...
	if (args.length >= 2) {
		...
		if (platform === null) {
			msg.reply('Invalid platform! Valid options are: xone, uplay, or ps4');
			return;
		}
	} else {
		// todo add usage
	...
	if (args.length === 3) {
		...
		if (gamemode === null) {
			msg.reply('Invalid gamemode! Valid options are: overall, ranked, or casual');
			return;
		}
	} else {
		gamemode = "general";
	...
	this.api.getPlayer(player, platform)
		.then(function(res) {
			...
			if (gamemode === "general") {
				...
				deaths = res.stats.ranked.deaths + res.stats.ranked.deaths;
				wins = res.stats.ranked.wins + res.stats.casual.wins;
				losses = res.stats.ranked.losses + res.stats.ranked.losses;
